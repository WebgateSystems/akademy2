...
module Api
  module V1
    module Management
      class ListParents < BaseInteractor
        def call
          authorize!
          load_parents
        end
        ...
        def load_parents
          ...
          base_query = base_query.preload(:school)
                                 .order(created_at: :desc)

          total_count = base_query.count
          parents = base_query.limit(per_page).offset(offset)
        ...
        def build_base_query
          User.joins(:user_roles)
              ...
              .where(user_roles: { school_id: school.id }, roles: { key: 'parent' })
              .distinct
        end
