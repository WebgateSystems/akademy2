...
RSpec.describe Api::V1::Management::LockTeacher do
  ...
  describe '#call' do
    context 'when user is authorized' do
      ...
      it 'locks the teacher account' do
      ...
      it 'unlocks the teacher account if already locked' do
        teacher.update!(locked_at: Time.current, failed_attempts: 5)
        result = described_class.call(context)

        expect(result).to be_success
        expect(result.form[:message]).to include('odblokowane')
        expect(teacher.reload.locked_at).to be_nil
        expect(teacher.reload.failed_attempts).to eq(0)
      end
