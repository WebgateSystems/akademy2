...
RSpec.describe 'API V1 Subjects', type: :request do
  ...
  describe 'GET /api/v1/subjects' do
    it 'returns 200' do
      allow(Api::V1::Subjects::ListSubjects).to receive(:call).and_return(success_result)
  ...
  describe 'GET /api/v1/subjects/:id' do
    it 'returns 200' do
      allow(Api::V1::Subjects::ShowSubject).to receive(:call).and_return(success_result(form: {}))
      get "/api/v1/subjects/#{SecureRandom.uuid}", headers: headers
      expect(response).to have_http_status(:ok)
    end

    it 'returns 404 when not found' do
      result = double(status: :not_found, success?: false, message: 'Not found')
      allow(Api::V1::Subjects::ShowSubject).to receive(:call).and_return(result)
      get "/api/v1/subjects/#{SecureRandom.uuid}", headers: headers
      expect(response).to have_http_status(:not_found)
    end

    it 'returns 401 without token' do
      get "/api/v1/subjects/#{SecureRandom.uuid}"
      expect(response).to have_http_status(:unauthorized)
    end
  end

  describe 'GET /api/v1/subjects/with_contents' do
    it 'returns 200' do
      allow(Api::V1::Subjects::ListSubjectsWithContents).to receive(:call).and_return(success_result)
      get '/api/v1/subjects/with_contents', headers: headers
      expect(response).to have_http_status(:ok)
    end
  end
end
